...
try {
    ...
    if (fs.existsSync('data.json')) {
        oficialData = JSON.parse(fs.readFileSync('data.json', 'utf8'));
    }

    // 1. Mapeamento Inteligente: Cria uma assinatura para o que já está online
    ...
    oficialData.forEach(p => {
        ...
        // Avalia o slug antigo ou novo e converte para o ID fixo do defeito
        if (s.includes('face-id')) defId = 'face-id';
        else if (s.includes('ci-carga') || s.includes('tristar')) defId = 'ci-carga';
        else if (s.includes('audio-ic')) defId = 'audio-ic';
        else if (s.includes('baseband') || s.includes('bpic')) defId = 'baseband';
        else if (s.includes('camera')) defId = 'camera-ldo';
        else if (s.includes('recuperacao')) defId = 'recuperacao-dados';
        else if (s.includes('reparo-placa-') || s.includes('conserto-placa-') || s.includes('vddmain')) defId = 'placa-morta';
    ...
    // 2. Processa todas as combinações
    modelos.forEach(modelo => {
        bairros.forEach(bairro => {
            ...
            defeitos.forEach(def => {
                ...
                // 3. Subtração baseada na assinatura, ignorando a URL
                const assinaturaAtual = `${modelo}|${bairro}|${atualDef.id}`;

                if (!assinaturasOnline.has(assinaturaAtual)) {
                    const slugGerado = `${atualDef.slug_prefix}-${modLimpo}-${bairroLimpo}`;
                    
                    paginasFaltantes.push({
                        slug: slugGerado,
    ...
    // 4. Salva APENAS as rotas inéditas
    fs.writeFileSync('placas_master.json', JSON.stringify(paginasFaltantes, null, 2));

    console.log('\n--- VARREDURA ESTRUTURAL CONCLUÍDA ---');
    console.log(`Malha teórica total: ${totalPossivel} páginas.`);
    console.log(`Identificadas no data.json: ${oficialData.length}`);
    console.log(`Páginas faltantes reais geradas: ${paginasFaltantes.length}`);
    console.log('----------------------------------------\n');

} catch (err) {
    console.error('Falha no circuito:', err);
}